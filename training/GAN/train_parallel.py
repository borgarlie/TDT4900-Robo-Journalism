...
from models.GAN.generator_seqgan_strat_parallel import p_monte_carlo_expansion
...
def train_GAN_parallel(config, generator, discriminator, training_pairs, eval_pairs, max_article_length, max_abstract_length,
              writer, gpu_device):
    ...
    # Make the process stop early
    # num_batches = 12

    # Initialise workers for num_monte_carlo_samples
    num_processes = generator.num_monte_carlo_samples  # (4 in this case)
    processes = []
    process_connections = []
    master_connections = []
    # Run monte carlo samples on one process each
    args = (max_sample_length, generator.vocabulary, generator.decoder, generator.batch_size)
    for rank in range(num_processes):
        process_conn, master_conn = mp.Pipe()
        p = mp.Process(target=p_monte_carlo_expansion, args=args + (rank, master_conn, gpu_device))
        p.start()
        processes.append(p)
        process_connections.append(process_conn)
            ...
            for n in range(n_generator):
                ...
                loss, mle_loss, policy_loss, policy_log_sum, reward, baseline, adjusted_reward \
                    = generator.train_on_batch(input_variable, full_input_variable, input_lengths, full_target_var,
                                               ...
                                               extended_vocabs, full_target_var_2, process_connections)
    ...
    log_message("Exiting workers and joining")

    # send exit signal to workers
    for rank in range(0, len(process_connections)):
        process_connections[rank].send(None)
